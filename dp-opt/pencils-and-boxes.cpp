...
ll a[1000006], b[500005], dp[500005];
...
void solve() {
  ...
  {
    vector<ll> comp;
    for (ll i = 1; i <= n; ++i) comp.push_back(a[i]);
    for (ll i = 1; i <= n; ++i) comp.push_back(b[i]);
    ...
    map<ll, ll> mp;
    for (ll i = 0; i < comp.size(); ++i) mp[comp[i]] = i;

    for (ll i = 1; i <= n; ++i) a[i] = mp[a[i]];
    for (ll i = 1; i <= n; ++i) b[i] = mp[b[i]];

    dbg("compression done");
  }  // Compression

  for (ll i = 1; i <= n; ++i) {
    ll t = i - k;
    if (t >= 0) {
      st.update(a[t + 1], a[t + 1], dp[t]);
    }
    ll sum = st.query(b[i], 1000006).val;
    if (sum >= 1) dp[i] = 1;
  }
